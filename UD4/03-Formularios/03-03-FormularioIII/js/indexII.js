...
document.addEventListener("DOMContentLoaded", () => {
  ...
  inputs.forEach(elemento=>{
    elemento.addEventListener("input", validarInputsEvento);
  })
  //establecer evento "change" de los radios
  tipo.forEach(elemento=>{
    elemento.addEventListener("change", validarTipoEvento);
  })
})
...
const validarInputsEvento=(e)=>{
  ...
  const error = document.getElementById(`err${e.target.id}`);
